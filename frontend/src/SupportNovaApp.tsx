...
const COLORS = ['#D946EF', '#00D2FF', '#8B5CF6', '#3B82F6', '#EC4899', '#06B6D4', '#F59E0B', '#10B981']
  ...
  return <Page>
    ...
    <div className="dashboard-grid">
      <Panel className="span-2" title="Complaint volume" subtitle="By Python-validated category">
        {categories.length ? <ResponsiveContainer width="100%" height={255}><AreaChart data={categories}><defs><linearGradient id="fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#D946EF" stopOpacity={.45} /><stop offset="60%" stopColor="#8B5CF6" stopOpacity={.15} /><stop offset="100%" stopColor="#00D2FF" stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="4 4" vertical={false} stroke="rgba(255, 255, 255, 0.06)" /><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#94A3B8' }} /><YAxis allowDecimals={false} axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#94A3B8' }} /><Tooltip contentStyle={{ backgroundColor: 'rgba(10, 14, 28, 0.95)', borderColor: 'rgba(217, 70, 239, 0.45)', borderRadius: '12px', color: '#fff', boxShadow: '0 0 25px rgba(217, 70, 239, 0.3)' }} /><Area type="monotone" dataKey="value" stroke="#D946EF" strokeWidth={2.8} fill="url(#fill)" /></AreaChart></ResponsiveContainer> : <EmptyState icon={BarChart3} title="No data yet" description="Analytics populate after complaints are analyzed." />}
      </Panel>
...
function ReportsPage() {
  ...
  return <Page>
    ...
    <div className="dashboard-grid">
      <Panel className="span-2" title="Department workload" subtitle="Complaints routed by Python rules"><ResponsiveContainer width="100%" height={310}><BarChart data={entries(data?.departments || {})} layout="vertical" margin={{ left: 20 }}><CartesianGrid horizontal={false} stroke="rgba(255, 255, 255, 0.06)" /><XAxis type="number" allowDecimals={false} tick={{ fontSize: 11, fill: '#94A3B8' }} /><YAxis dataKey="name" type="category" width={130} tick={{ fontSize: 11, fill: '#94A3B8' }} /><Tooltip contentStyle={{ backgroundColor: 'rgba(10, 14, 28, 0.95)', borderColor: 'rgba(0, 210, 255, 0.45)', borderRadius: '12px', color: '#fff', boxShadow: '0 0 25px rgba(0, 210, 255, 0.3)' }} /><Bar dataKey="value" fill="#00D2FF" radius={[0, 6, 6, 0]} barSize={18} /></BarChart></ResponsiveContainer></Panel>
...
function Brand({ light }: { light?: boolean }) { return <div className={`brand ${light ? 'brand-light' : ''}`}><div className="brand-mark brand-logo-wrap"><img src="/logo.png" alt="SupportNova" className="brand-logo-img" /></div><div><span>SupportNova</span><small>ResponseX Intelligence</small></div></div> }
