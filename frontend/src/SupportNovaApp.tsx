...
function Intelligence({ title, icon: Icon, data, verified }: { title: string; icon: typeof Bot; data: ComplaintIntelligence; verified?: boolean }) { const items = [['Category', data.issue_category], ['Subcategory', data.subcategory], ['Department', data.department], ['Urgency', data.urgency], ['Priority', data.priority], ['Escalation', data.escalation_required ? data.escalation_level || 'Required' : 'Not required'], ['Policy', data.policy_id]]; return <div className={verified ? 'verified-column' : ''}><h3><Icon />{title}{verified && <CheckCircle2 />}</h3>{items.map(([k, v]) => <p key={String(k)}><span>{String(k)}</span><b>{text(v)}</b></p>)}</div> }
